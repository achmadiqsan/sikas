                    ...
                    <div class="panel panel-default">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <tbody>
                                    	...
                                    	$no = 1;
                                    	$total = 0;
                                    	$sql=mysqli_query($con,"SELECT * FROM transaksi WHERE jenis='Masuk'");
                                        ...
                                        <tr class="odd gradeX">
            								...
            								<td>
            									<a id="edit_data" data-toggle="modal" data-target="#edit" data-kode="<?= $data['kode'] ?>" data-ket="<?= $data['keterangan'] ?>" data-tgl="<?= $data['tgl'] ?>" data-jml="<?= $data['jumlah'] ?>" href="#" class="btn btn-info"><i class="fa fa-edit"></i>Edit</a>

            									<a onclick="return confirm('Yakin Akan Menghapus Data Ini ??')" href="?page=masuk&aksi=hapus&kode=<?= $data['kode']; ?>" class="btn btn-danger"><i class="fa fa-trash"></i>Hapus</a>
            								</td>
                                        </tr>
                                        <?php 
	                                    ...
	                                    <tr>
	                                    	<td colspan="4"><strong>Total Kas Masuk</strong></td>
                     ...
                     <?php 
                        if (isset($_POST['simpan'])) {
                            ...
                            $sql = "INSERT INTO transaksi (kode, keterangan, tgl, jumlah, jenis, keluar) VALUES ('$kode', '$ket', '$tgl', '$jml', 'Masuk', 0)";
                            $query = mysqli_query($con, $sql);
                            if ($query) {
                                echo "<script>alert('Data berhasil ditambahkan!');window.location.href='?page=masuk'</script>";
                            } else {
                      ...
                      <div class="panel-body">
                        ...
                        <?php 
                            ...
                            $sql = "UPDATE `transaksi` SET `keterangan`='$ket',`tgl`='$tgl',`jumlah`='$jml' WHERE `kode`='$kode'";
                            $query = mysqli_query($con, $sql);
                            if ($query) {
                                echo "<script>alert('Data berhasil diubah!');window.location.href='?page=masuk'</script>";
                            } else {
                                echo "Error : " . mysqli_error($con);
                            }
                        }   
